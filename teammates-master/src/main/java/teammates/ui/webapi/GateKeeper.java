...
final class GateKeeper {
    ...
    // These methods ensures that the nominal user specified has access to a given FeedbackQuestionTest
    ...
    // These methods ensures that the nominal user specified can perform the specified action on a given FeedbackQuestionTest.

    private void verifyNotNull(Object object, String typeName)
            throws UnauthorizedAccessException {
        if (object == null) {
            throw new UnauthorizedAccessException("Trying to access system using a non-existent " + typeName + " FeedbackQuestionTest");
        }
    }
